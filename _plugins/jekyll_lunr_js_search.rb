  ...
  module LunrJsSearch
    class Indexer < Jekyll::Generator
      def initialize(config = {})
        ...
        require_js = File.join(File.dirname(__FILE__), "../js/require.js")
        @require_js_path = File.exist?(require_js) ? require_js : File.join(@js_dir, File.basename(require_js))
        raise "Could not find #{@require_js_path}" if !File.exist?(@require_js_path)
        ...
        tiny_seg = File.join(File.dirname(__FILE__), "../js/tinyseg.js")
        @tiny_seg_path = File.exist?(tiny_seg) ? tiny_seg : File.join(@js_dir, File.basename(tiny_seg))
        raise "Could not find #{@tiny_seg_path}" if !File.exist?(@tiny_seg_path)

        lunr_stem = File.join(File.dirname(__FILE__), "../js/lunr.stemmer.support.js")
        @lunr_stem_path = File.exist?(lunr_stem) ? lunr_stem : File.join(@js_dir, File.basename(lunr_stem))
        raise "Could not find #{@lunr_stem_path}" if !File.exist?(@lunr_stem_path)

        lunr_jp = File.join(File.dirname(__FILE__), "../js/lunr.jp.js")
        @lunr_jp_path = File.exist?(lunr_jp) ? lunr_jp : File.join(@js_dir, File.basename(lunr_jp))
        raise "Could not find #{@lunr_jp_path}" if !File.exist?(@lunr_jp_path)

        lunr_multi = File.join(File.dirname(__FILE__), "../js/lunr.multi.js")
        @lunr_multi_path = File.exist?(lunr_multi) ? lunr_multi : File.join(@js_dir, File.basename(lunr_multi))
        raise "Could not find #{@lunr_multi_path}" if !File.exist?(@lunr_multi_path)

        ctx = V8::Context.new
        ctx.load(@require_js_path)
        # ctx.load(@lunr_path)
        lunr = ctx["require('lunr.min.js')"]
        # ctx.eval("var lunr = require('lunr.min.js')")
        ctx.load(@tiny_seg_path)
        ctx.load(@lunr_stem_path)
        ctx.load(@lunr_jp_path)
        ctx.load(@lunr_multi_path)
        ctx.eval("lunr.multiLanguage('en', 'jp')")
