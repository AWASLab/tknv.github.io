---
layout: post
title: "ANSIBLE - Your very own server"
date: Wednesay, 4. March 2015 12:32AM 
author: tknv
...
+    - postgresql-9.4
     ...
     - python-psycopg2
     - sasl2-bin
diff --git a/roles/monitoring/files/etc_monit_conf.d_pgsql b/roles/monitoring/files/etc_monit_conf.d_pgsql
index 82d377a..20ae401 100644
...
diff --git a/roles/owncloud/tasks/owncloud.yml b/roles/owncloud/tasks/owncloud.yml
index 9794299..a9d00cc 100644
...
```
openssl req -nodes -newkey rsa:2048 -keyout roles/common/files/wildcard_private.key -out mycert.csr.pem
openssl req -new -x509 -days 3650 -in mycert.csr -key roles/common/files/wildcard_private.key -out roles/common/files/wildcard_public_cert.crt -config roles/common/files/openssl.cnf
cp roles/common/files/wildcard_public_cert.crt roles/common/files/wildcard_ca.pem
```

## Prepare server
Hosting servers; Own PC, Virtual Box or VPS( Linode,  さくら-very cost effective)
About vars(vars/user.yml), see [sovereign](https://github.com/al3x/sovereign)   
at server, add postgresql repo for 9.4.  
```
...
sudo ./install_fusionpbx.sh install-both user pbx.YOUR.DOMAIN
```
